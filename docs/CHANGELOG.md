...
`server`方只要排好一个块就发送出去，这时从队列中取出一个端口号用于发送。先发送当前块中的数组长度，随后再发数据（因为未必是严格的`BUFFER_SIZE`）。发送完毕后，需要等待`client`回复发送完毕的信号后，将端口号重新放回数组，表示可用。注意TCP连接的建立应该在一开始，对队列的所有操作都要加锁！。
